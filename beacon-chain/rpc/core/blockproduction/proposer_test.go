...
import (
	...
	dbpkg "github.com/OffchainLabs/prysm/v7/beacon-chain/db"
...
func TestServer_GetBeaconBlock_Phase0(t *testing.T) {
	...
	proposerServer := getBlockProducer(db, beaconState, parentRoot[:])
	...
	block, err := proposerServer.ProduceBlock(ctx, 1, randaoReveal, graffiti[:], false, DefaultBuilderBoostFactor)
	require.NoError(t, err)
...
func TestServer_GetBeaconBlock_Altair(t *testing.T) {
	...
	block, err := proposerServer.ProduceBlock(ctx, altairSlot+1, randaoReveal, graffiti[:], false, DefaultBuilderBoostFactor)
	require.NoError(t, err)
...
func TestServer_GetBeaconBlock_Bellatrix(t *testing.T) {
	...
	block, err := proposerServer.ProduceBlock(ctx, bellatrixSlot+1, randaoReveal, graffiti[:], false, DefaultBuilderBoostFactor)
	require.NoError(t, err)
	...
	params.OverrideBeaconConfig(cfg)
	_, err = proposerServer.ProduceBlock(ctx, bellatrixSlot+1, randaoReveal, graffiti[:], false, DefaultBuilderBoostFactor)
	require.NoError(t, err)
...
func TestServer_GetBeaconBlock_Capella(t *testing.T) {
	...
	got, err := proposerServer.ProduceBlock(ctx, capellaSlot+1, randaoReveal, graffiti[:], false, DefaultBuilderBoostFactor)
	require.NoError(t, err)
...
func TestServer_GetBeaconBlock_Deneb(t *testing.T) {
	...
	got, err := proposerServer.ProduceBlock(ctx, denebSlot+1, randaoReveal, graffiti[:], false, DefaultBuilderBoostFactor)
	require.NoError(t, err)
...
func TestServer_GetBeaconBlock_Electra(t *testing.T) {
	...
	_, err = proposerServer.ProduceBlock(ctx, electraSlot+1, randaoReveal, graffiti[:], false, DefaultBuilderBoostFactor)
	require.NoError(t, err)
}
...
func TestServer_GetBeaconBlock_Fulu(t *testing.T) {
	...
	_, err = proposerServer.ProduceBlock(ctx, fuluSlot+1, randaoReveal, graffiti[:], false, DefaultBuilderBoostFactor)
	require.NoError(t, err)
}
...
func TestServer_GetBeaconBlock_Optimistic(t *testing.T) {
	...
	graffiti := bytesutil.ToBytes32([]byte("eth2"))
	_, err = proposerServer.ProduceBlock(t.Context(), bellatrixSlot+1, nil, graffiti[:], false, DefaultBuilderBoostFactor)
	require.ErrorContains(t, errOptimisticMode.Error(), err)
}

func getBlockProducer(db dbpkg.Database, headState state.BeaconState, headRoot []byte) *BlockProducer {
	...
	return &BlockProducer{
		...
		BlockBuilderClient:     &builderTest.MockBuilderService{HasConfigured: true},
	}
}

func injectSlashings(t *testing.T, st state.BeaconState, keys []bls.SecretKey, bp *BlockProducer) ([]*ethpb.ProposerSlashing, []*ethpb.AttesterSlashing) {
	...
	for i := primitives.ValidatorIndex(0); uint64(i) < params.BeaconConfig().MaxProposerSlashings; i++ {
		...
		err = bp.SlashingsPool.InsertProposerSlashing(t.Context(), st, proposerSlashing)
		require.NoError(t, err)
	}
	...
	for i := uint64(0); i < params.BeaconConfig().MaxAttesterSlashings; i++ {
		...
		err = bp.SlashingsPool.InsertAttesterSlashing(t.Context(), st, generatedAttesterSlashing.(*ethpb.AttesterSlashing))
		require.NoError(t, err)
	}
...
func TestProposer_ComputeStateRoot_OK(t *testing.T) {
	...
	require.NoError(t, err)
	_, err = bp.computeStateRoot(t.Context(), wsb)
	require.NoError(t, err)
}
...
func TestHandleStateRootError_MaxAttemptsReached(t *testing.T) {
	...
	_, err = bp.handleStateRootError(ctx, wsb, transition.ErrAttestationsSignatureInvalid)
...
func TestProposer_PendingDeposits_Eth1DataVoteOK(t *testing.T) {
	...
	_, eth1Height, err := bp.canonicalEth1Data(ctx, beaconState, &ethpb.Eth1Data{})
	require.NoError(t, err)
	...
	_, eth1Height, err = bp.canonicalEth1Data(ctx, beaconState, vote)
	require.NoError(t, err)
...
func TestProposer_PendingDeposits_OutsideEth1FollowWindow(t *testing.T) {
	...
	deposits, err := bp.deposits(ctx, beaconState, &ethpb.Eth1Data{})
	require.NoError(t, err)
...
func TestProposer_PendingDeposits_FollowsCorrectEth1Block(t *testing.T) {
	...
	deposits, err = bp.deposits(ctx, beaconState, vote)
	require.NoError(t, err)
...
func TestProposer_DepositTrie_UtilizesCachedFinalizedDeposits(t *testing.T) {
	...
	bs := &BlockProducer{
		...
		DepositFetcher:         depositCache,
		PendingDepositsFetcher: depositCache,
		Proposer:               &BlockProposerDeps{BlockReceiver: &mock.ChainService{State: beaconState, Root: blkRoot[:]}},
		HeadFetcher:            &mock.ChainService{State: beaconState, Root: blkRoot[:]},
	}
