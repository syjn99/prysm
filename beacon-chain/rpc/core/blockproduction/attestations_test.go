...
func Test_packAttestations(t *testing.T) {
	...
	bp := &BlockProducer{AttPool: pool, HeadFetcher: &chainMock.ChainService{}, TimeFetcher: &chainMock.ChainService{Slot: &slot}}

	t.Run("Phase 0", func(t *testing.T) {
		st, _ := util.DeterministicGenesisState(t, 64)
		require.NoError(t, st.SetSlot(1))

		atts, err := bp.packAttestations(ctx, st, 0)
		require.NoError(t, err)
	...
	t.Run("Electra", func(t *testing.T) {
		...
		atts, err := bp.packAttestations(ctx, st, params.BeaconConfig().SlotsPerEpoch)
		require.NoError(t, err)
		require.Equal(t, 1, len(atts))
...
func Benchmark_packAttestations_Electra(b *testing.B) {
	...
	for b.Loop() {
		_, err = bp.packAttestations(ctx, st, params.BeaconConfig().SlotsPerEpoch+1)
		require.NoError(b, err)
	}
}
...
func Test_isAttestationFromCurrentEpoch(t *testing.T) {
	...
	require.Equal(t, true, bp.isAttestationFromCurrentEpoch(a, epoch))

	a.Data.Target.Epoch = 1
	require.Equal(t, false, bp.isAttestationFromCurrentEpoch(a, epoch))
}
...
func Test_isAttestationFromPreviousEpoch(t *testing.T) {
	...
	require.Equal(t, true, bp.isAttestationFromPreviousEpoch(a, epoch))

	a.Data.Target.Epoch = 1
	require.Equal(t, false, bp.isAttestationFromPreviousEpoch(a, epoch))
}
...
func Test_filterCurrentEpochAttestationByTarget(t *testing.T) {
	...
	got, err := bp.filterCurrentEpochAttestationByTarget(a, targetRoot, 1, epoch)
	require.NoError(t, err)
	require.Equal(t, true, got)

	got, err = bp.filterCurrentEpochAttestationByTarget(a, [32]byte{}, 1, epoch)
	require.NoError(t, err)
	require.Equal(t, false, got)

	got, err = bp.filterCurrentEpochAttestationByTarget(a, targetRoot, 2, epoch)
	require.NoError(t, err)
	require.Equal(t, false, got)
...
func Test_filterPreviousEpochAttestationByTarget(t *testing.T) {
	...
	got, err := bp.filterPreviousEpochAttestationByTarget(a, &ethpb.Checkpoint{
		Epoch: 1,
		Root:  targetRoot[:],
	}, epoch)
	require.NoError(t, err)
...
func Test_filterCurrentEpochAttestationByForkchoice(t *testing.T) {
	...
	ctx := t.Context()
	got, err := bp.filterCurrentEpochAttestationByForkchoice(ctx, a, epoch)
	require.NoError(t, err)
	require.Equal(t, false, got)

	a.Data.BeaconBlockRoot = targetRoot[:]
	bp.ForkchoiceFetcher = &chainMock.ChainService{BlockSlot: 1}
	got, err = bp.filterCurrentEpochAttestationByForkchoice(ctx, a, epoch)
	require.NoError(t, err)
	require.Equal(t, true, got)

	bp.ForkchoiceFetcher = &chainMock.ChainService{BlockSlot: 100}
	got, err = bp.filterCurrentEpochAttestationByForkchoice(ctx, a, epoch)
	require.NoError(t, err)
	require.Equal(t, false, got)
