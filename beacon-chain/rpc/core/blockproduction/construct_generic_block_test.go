...
func TestConstructGenericBeaconBlock(t *testing.T) {
	...
	t.Run("NilBlock", func(t *testing.T) {
		_, err := bp.constructGenericBeaconBlock(nil, nil, primitives.ZeroWei())
	...
	t.Run("fulu block", func(t *testing.T) {
		...
		result, err := bp.constructGenericBeaconBlock(b, bundle, primitives.ZeroWei())
		require.NoError(t, err)
	...
	t.Run("electra block", func(t *testing.T) {
		...
		r1, err := eb.Block.HashTreeRoot()
		require.NoError(t, err)
		result, err := bp.constructGenericBeaconBlock(b, nil, primitives.ZeroWei())
		require.NoError(t, err)
	...
	t.Run("blind deneb block", func(t *testing.T) {
		...
		result, err := bp.constructGenericBeaconBlock(b, scs, primitives.ZeroWei())
		require.NoError(t, err)
