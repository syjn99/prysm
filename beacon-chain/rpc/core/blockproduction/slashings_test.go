...
func TestBlockProducer_getSlashings(t *testing.T) {
	...
	for i := primitives.ValidatorIndex(0); uint64(i) < params.BeaconConfig().MaxProposerSlashings; i++ {
		...
		err = bp.SlashingsPool.InsertProposerSlashing(t.Context(), beaconState, proposerSlashing)
		require.NoError(t, err)
	}
	...
	for i := uint64(0); i < params.BeaconConfig().MaxAttesterSlashings; i++ {
		...
		err = bp.SlashingsPool.InsertAttesterSlashing(t.Context(), beaconState, attesterSlashing)
		require.NoError(t, err)
	}

	p, a := bp.getSlashings(t.Context(), beaconState)
