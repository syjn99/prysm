...
func TestProposer_GetSyncAggregate_OK(t *testing.T) {
	...
	require.NoError(t, err)
	assert.DeepEqual(t, []byte{0xf, 0xf, 0xf, 0xf}, []byte(aggregate.SyncCommitteeBits))

	aggregate, err = proposerServer.getSyncAggregate(t.Context(), 2, bytesutil.ToBytes32(conts[0].BlockRoot), st)
	require.NoError(t, err)
	assert.DeepEqual(t, []byte{0xaa, 0xaa, 0xaa, 0xaa}, []byte(aggregate.SyncCommitteeBits))

	aggregate, err = proposerServer.getSyncAggregate(t.Context(), 3, bytesutil.ToBytes32(conts[0].BlockRoot), st)
	require.NoError(t, err)
	assert.DeepEqual(t, []byte(bitfield.NewBitvector32()), []byte(aggregate.SyncCommitteeBits))
}

func TestBlockProducer_SetSyncAggregate_EmptyCase(t *testing.T) {
	b, err := blocks.NewSignedBeaconBlock(util.NewBeaconBlockAltair())
	require.NoError(t, err)
	bp := &BlockProducer{} // BlockProducer is not initialized with sync committee pool.
	bp.setSyncAggregate(t.Context(), b, nil)
