...
func (b *BlockProducer) getPayloadHeaderFromBuilder(
	ctx context.Context,
	...
	headBlk, err := b.HeadFetcher.HeadBlock(ctx)
	if err != nil {
		return nil, err
	}

	h, err := headBlk.Block().Body().Execution()
	if err != nil {
		return nil, errors.Wrap(err, "failed to get execution header")
	}
	pk, err := b.HeadFetcher.HeadValidatorIndexToPublicKey(ctx, idx)
	if err != nil {
		return nil, err
	}
	...
	signedBid, err := b.BlockBuilderClient.GetHeader(ctx, slot, bytesutil.ToBytes32(h.BlockHash()), pk)
	if err != nil {
		return nil, err
	}
	...
	reg, err := b.BlockBuilderClient.RegistrationByValidatorID(ctx, idx)
	if err != nil {
