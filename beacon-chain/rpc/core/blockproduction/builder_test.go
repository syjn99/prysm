...
func TestBlockProducer_circuitBreakBuilder(t *testing.T) {
...
func TestBlockProducer_validatorRegistered(t *testing.T) {
	b, err := builder.NewService(t.Context())
	require.NoError(t, err)
	proposerServer := &BlockProducer{
		BlockBuilderClient: b,
	}
	ctx := t.Context()
	...
	require.NoError(t, err)
	proposerServer.BlockBuilderClient = realBuilder
	reg, err = proposerServer.validatorRegistered(ctx, 0)
	require.NoError(t, err)
	require.Equal(t, false, reg)
...
func TestBlockProducer_canUseBuilder(t *testing.T) {
	proposerServer := &BlockProducer{
		BlockBuilderClient: &testing2.MockBuilderService{
			HasConfigured: false,
		},
	}
	...
	proposerServer.BlockBuilderClient = &testing2.MockBuilderService{
		HasConfigured: true,
