...
func (b *BlockProducer) eth1DataMajorityVote(ctx context.Context, beaconState state.BeaconState) (*ethpb.Eth1Data, error) {
	...
	votingPeriodStartTime := b.slotStartTime(slot)

	if b.MockEth1Votes {
		return b.mockETH1DataVote(ctx, slot)
	}
	if !b.Eth1InfoFetcher.ExecutionClientConnected() {
		return b.randomETH1DataVote(ctx)
	}
	eth1DataNotification = false

	genesisTime, _ := b.Eth1InfoFetcher.GenesisExecutionChainInfo()
	...
	if latestValidTime < genesisTime+followDistanceSeconds {
		...
		return b.HeadFetcher.HeadETH1Data(), nil
	}

	lastBlockByLatestValidTime, err := b.Eth1BlockFetcher.BlockByTimestamp(ctx, latestValidTime)
	if err != nil {
	...
	lastBlockDepositCount, lastBlockDepositRoot := b.DepositFetcher.DepositsNumberAndRootAtHeight(ctx, lastBlockByLatestValidTime.Number)
	if lastBlockDepositCount == 0 {
		return b.ChainStartFetcher.ChainStartEth1Data(), nil
	}

	if lastBlockDepositCount >= b.HeadFetcher.HeadETH1Data().DepositCount {
		h, err := b.Eth1BlockFetcher.BlockHashByHeight(ctx, lastBlockByLatestValidTime.Number)
		if err != nil {
...
func (b *BlockProducer) slotStartTime(slot primitives.Slot) uint64 {
	startTime, _ := b.Eth1InfoFetcher.GenesisExecutionChainInfo()
...
func (b *BlockProducer) canonicalEth1Data(
	ctx context.Context,
	...
	_, canonicalEth1DataHeight, err := b.Eth1BlockFetcher.BlockExists(ctx, eth1BlockHash)
	if err != nil {
...
func (b *BlockProducer) mockETH1DataVote(ctx context.Context, slot primitives.Slot) (*ethpb.Eth1Data, error) {
	...
	headState, err := b.HeadFetcher.HeadStateReadOnly(ctx)
	if err != nil {
		return nil, err
	}
	var enc []byte
...
func (b *BlockProducer) randomETH1DataVote(ctx context.Context) (*ethpb.Eth1Data, error) {
	if !eth1DataNotification {
