...
func TestServer_setExecutionData(t *testing.T) {
	...
	vs := &BlockProducer{
		...
		BlockBuilderClient:     &builderTest.MockBuilderService{HasConfigured: true, Cfg: &builderTest.Config{BeaconDB: beaconDB}},
	...
	t.Run("Builder configured. Builder Block has higher value. Incorrect withdrawals", func(t *testing.T) {
		...
		vs.BlockBuilderClient = &builderTest.MockBuilderService{
			BidCapella:    sBid,
			HasConfigured: true,
...
func TestServer_getPayloadHeader(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			vs := &BlockProducer{BlockBuilderClient: tc.mock, HeadFetcher: tc.fetcher, TimeFetcher: &blockchainTest.ChainService{
				Genesis: genesis,
			}}
