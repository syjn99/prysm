...
func (b *BlockProducer) packAttestations(ctx context.Context, latestState state.BeaconState, blkSlot primitives.Slot) ([]ethpb.Att, error) {
	...
	if features.Get().EnableExperimentalAttestationPool {
		atts = b.AttestationCache.GetAll()
		atts = b.validateAndDeleteAttsInPool(ctx, latestState, atts)
	} else {
		atts = b.AttPool.AggregatedAttestations()
		atts = b.validateAndDeleteAttsInPool(ctx, latestState, atts)

		uAtts := b.AttPool.UnaggregatedAttestations()
		uAtts = b.validateAndDeleteAttsInPool(ctx, latestState, uAtts)
	...
	return b.filterAttestationBySignature(ctx, atts, latestState)
}
...
func (b *BlockProducer) validateAndDeleteAttsInPool(ctx context.Context, st state.BeaconState, atts []ethpb.Att) []ethpb.Att {
	...
	if err := b.deleteAttsInPool(ctx, invalidAtts); err != nil {
...
func (b *BlockProducer) deleteAttsInPool(ctx context.Context, atts []ethpb.Att) error {
	...
	for _, att := range atts {
		...
		if features.Get().EnableExperimentalAttestationPool {
			if err := b.AttestationCache.DeleteCovered(att); err != nil {
				return errors.Wrap(err, "could not delete attestation")
			}
		} else {
			if att.IsAggregated() {
				if err := b.AttPool.DeleteAggregatedAttestation(att); err != nil {
					return err
				}
			} else {
				if err := b.AttPool.DeleteUnaggregatedAttestation(att); err != nil {
					return err
				}
			}
		}
...
func (b *BlockProducer) isAttestationFromCurrentEpoch(att ethpb.Att, currentEpoch primitives.Epoch) bool {
...
func (b *BlockProducer) isAttestationFromPreviousEpoch(att ethpb.Att, currentEpoch primitives.Epoch) bool {
...
func (b *BlockProducer) filterCurrentEpochAttestationByForkchoice(ctx context.Context, att ethpb.Att, currentEpoch primitives.Epoch) (bool, error) {
	if !b.isAttestationFromCurrentEpoch(att, currentEpoch) {
		return false, nil
	}
	...
	slot, err := b.ForkchoiceFetcher.RecentBlockSlot(attBlockRoot)
	if err != nil {
		return false, err
	}
	...
	return b.ForkchoiceFetcher.IsCanonical(ctx, attBlockRoot)
}
...
func (b *BlockProducer) filterCurrentEpochAttestationByTarget(att ethpb.Att, targetRoot [32]byte, targetEpoch, currentEpoch primitives.Epoch) (bool, error) {
	if !b.isAttestationFromCurrentEpoch(att, currentEpoch) {
		return false, nil
	}
...
func (b *BlockProducer) filterPreviousEpochAttestationByTarget(att ethpb.Att, cp *ethpb.Checkpoint, currentEpoch primitives.Epoch) (bool, error) {
	if !b.isAttestationFromPreviousEpoch(att, currentEpoch) {
		return false, nil
	}
...
func (b *BlockProducer) filterAttestationBySignature(ctx context.Context, atts proposerAtts, st state.BeaconState) (proposerAtts, error) {
	headSlot := b.HeadFetcher.HeadSlot()
	targetEpoch := slots.ToEpoch(headSlot)
	r, err := b.HeadFetcher.HeadRoot(ctx)
	if err != nil {
		return nil, err
	}
	headRoot := [32]byte(r)

	targetRoot, err := b.HeadFetcher.TargetRootForEpoch(headRoot, targetEpoch)
	if err != nil {
		return nil, err
	}
	...
	prevTargetRoot, err := b.HeadFetcher.TargetRootForEpoch(headRoot, prevTargetEpoch)
	if err != nil {
		return nil, err
	}

	currentSlot := b.TimeFetcher.CurrentSlot()
	currentEpoch := slots.ToEpoch(currentSlot)

	var verifiedAtts proposerAtts
	var unverifiedAtts proposerAtts
	for _, att := range atts {
		ok, err := b.filterCurrentEpochAttestationByTarget(att, targetRoot, targetEpoch, currentEpoch)
		if err != nil {
		...
		ok, err = b.filterPreviousEpochAttestationByTarget(att, &ethpb.Checkpoint{Root: prevTargetRoot[:], Epoch: prevTargetEpoch}, currentEpoch)
		if err != nil {
		...
		ok, err = b.filterCurrentEpochAttestationByForkchoice(ctx, att, currentEpoch)
		if err != nil {
