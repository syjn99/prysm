...
func (b *BlockProducer) getLocalPayload(ctx context.Context, blk interfaces.ReadOnlyBeaconBlock, st state.BeaconState) (*consensusblocks.GetPayloadResponse, error) {
	...
	return b.getLocalPayloadFromEngine(ctx, st, headRoot, slot, vIdx)
}
...
func (b *BlockProducer) getLocalPayloadFromEngine(
	ctx context.Context,
	...
	payloadId, ok := b.PayloadIDCache.PayloadID(slot, parentRoot)

	val, tracked := b.TrackedValidatorsCache.Validator(proposerId)
	if !tracked {
	...
	if ok && payloadId != [8]byte{} {
		...
		res, err := b.ExecutionEngineCaller.GetPayload(ctx, pid, slot)
		if err == nil {
	...
	parentHash, err := b.getParentBlockHash(ctx, st, slot)
	switch {
	...
	if st.Version() >= version.Bellatrix {
		finalizedBlockHash = b.FinalizationFetcher.FinalizedBlockHash()
		justifiedBlockHash = b.FinalizationFetcher.UnrealizedJustifiedPayloadBlockHash()
	}
	...
	payloadID, _, err := b.ExecutionEngineCaller.ForkchoiceUpdated(ctx, f, attr)
	if err != nil {
	...
	res, err := b.ExecutionEngineCaller.GetPayload(ctx, *payloadID, slot)
	if err != nil {
		return nil, err
	}
...
func (b *BlockProducer) getTerminalBlockHashIfExists(ctx context.Context, transitionTime uint64) ([]byte, bool, error) {
	terminalBlockHash := params.BeaconConfig().TerminalBlockHash
	// Terminal block hash override takes precedence over terminal total difficulty.
	if params.BeaconConfig().TerminalBlockHash != params.BeaconConfig().ZeroHash {
		exists, _, err := b.Eth1BlockFetcher.BlockExists(ctx, terminalBlockHash)
		if err != nil {
	...
	return b.ExecutionEngineCaller.GetTerminalBlockHash(ctx, transitionTime)
}

func (b *BlockProducer) getBuilderPayloadAndBlobs(ctx context.Context,
	slot primitives.Slot,
	...
	canUseBuilder, err := b.canUseBuilder(ctx, slot, vIdx)
	if err != nil {
	...
	return b.getPayloadHeaderFromBuilder(ctx, slot, vIdx, parentGasLimit)
}
...
func (b *BlockProducer) getParentBlockHash(ctx context.Context, st state.BeaconState, slot primitives.Slot) ([]byte, error) {
	...
	return getParentBlockHashPreMerge(ctx, b, st, slot)
}
...
func getParentBlockHashPreMerge(ctx context.Context, bb *BlockProducer, st state.BeaconState, slot primitives.Slot) ([]byte, error) {
	t, err := slots.StartTime(st.GenesisTime(), slot)
	if err != nil {
		return nil, err
	}

	parentHash, hasTerminalBlock, err := bb.getTerminalBlockHashIfExists(ctx, uint64(t.Unix()))
	if err != nil {
		return nil, err
	}
	if !hasTerminalBlock {
