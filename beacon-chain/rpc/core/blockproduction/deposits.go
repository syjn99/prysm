...
func (b *BlockProducer) packDepositsAndAttestations(
	ctx context.Context,
	...
	eg.Go(func() error {
		...
		localDeposits, err := b.deposits(egctx, head, eth1Data)
		if err != nil {
	...
	eg.Go(func() error {
		...
		localAtts, err := b.packAttestations(egctx, head, blkSlot)
		if err != nil {
...
func (b *BlockProducer) deposits(
	...
	canonicalEth1Data, canonicalEth1DataHeight, err := b.canonicalEth1Data(ctx, beaconState, currentVote)
	if err != nil {
		return nil, err
	}

	_, genesisEth1Block := b.Eth1InfoFetcher.GenesisExecutionChainInfo()
	if genesisEth1Block.Cmp(canonicalEth1DataHeight) == 0 {
		return []*ethpb.Deposit{}, nil
	}

	// If there are no pending deposits, exit early.
	allPendingContainers := b.PendingDepositsFetcher.PendingContainers(ctx, canonicalEth1DataHeight)
	if len(allPendingContainers) == 0 {
		log.Debug("No pending deposits for inclusion in block")
		return []*ethpb.Deposit{}, nil
	}

	depositTrie, err := b.depositTrie(ctx, canonicalEth1Data, canonicalEth1DataHeight)
	if err != nil {
...
func (b *BlockProducer) depositTrie(ctx context.Context, canonicalEth1Data *ethpb.Eth1Data, canonicalEth1DataHeight *big.Int) (cache.MerkleTree, error) {
	ctx, span := trace.StartSpan(ctx, "ProposerServer.depositTrie")
	defer span.End()

	var depositTrie cache.MerkleTree

	finalizedDeposits, err := b.DepositFetcher.FinalizedDeposits(ctx)
	if err != nil {
		return nil, err
	}
	depositTrie = finalizedDeposits.Deposits()
	upToEth1DataDeposits := b.DepositFetcher.NonFinalizedDeposits(ctx, finalizedDeposits.MerkleTrieIndex(), canonicalEth1DataHeight)
	...
	if shouldRebuildTrie(canonicalEth1Data.DepositCount, uint64(len(upToEth1DataDeposits))) {
		...
		return b.rebuildDepositTrie(ctx, canonicalEth1Data, canonicalEth1DataHeight)
	}
...
func (b *BlockProducer) rebuildDepositTrie(ctx context.Context, canonicalEth1Data *ethpb.Eth1Data, canonicalEth1DataHeight *big.Int) (cache.MerkleTree, error) {
	ctx, span := trace.StartSpan(ctx, "ProposerServer.rebuildDepositTrie")
	defer span.End()

	deposits := b.DepositFetcher.AllDeposits(ctx, canonicalEth1DataHeight)
